...
void UBuffComponent::RechargeShield(float ShieldAmount, float RechargeTime)
{
	...
	ShieldRechargeRate = ShieldAmount / RechargeTime;
	ShieldRechargeAmount += ShieldAmount;
}
...
{
	if (!bRechargingShield || Character == nullptr || Character->IsElimmed()) return;

	const float RechargeThisFrame = ShieldRechargeRate * DeltaTime;
	Character->SetShield(FMath::Clamp(Character->GetShield() + RechargeThisFrame, 0.f, Character->GetMaxShield()));
	Character->UpdateHUDShield();
	ShieldRechargeAmount -= RechargeThisFrame;
